...
class Calculator:
    def eval(self, expression):
        ...
        for char in expression:     
            if char not in allowed_inputs:  # Handling characters, other symbols (*-=/)
                return "ERROR: Unexpected Input: " + char + ". Input must be alphanumeric and addition only! Ex \"3 + 2\""
            if char == '+':
                if len(integer_one_string) == 0:
                ...
                continue
            if not flag_integer_one_complete: # first integer done, handles multiple ++++++, starts second integer
                integer_one_string += char
            else:   
                integer_two_string += char
            index += 1
        if not flag_integer_one_complete:
            return "ERROR: Addition operator not given!"
        if len(integer_two_string) == 0:    # Handles no 2nd integer, such as "2432+   "
